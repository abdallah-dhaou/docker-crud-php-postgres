...
if (isset($_POST['btn_signin'])) {
    if (! empty($_POST["login"])) {
        ...
        if ($isAuthenticated) {
            ...
            $util->redirect("crud/crud_read.php.php");
        } else {
...
if (isset($_POST['btn_signin'])) {
    ...
    $userInfo = $user->getUserByLogin(trim($_POST['login_signin']));

//    error_log("\n DUMP userResult verif_user". print_r( $userInfo, true ), 3, "/tmp/errors.log");
//    error_log("\n count  ".count($userInfo), 3, "/tmp/errors.log");
    if (count($userInfo) == 1 && password_verify(trim($_POST['password_signin']), $userInfo[0]['u_pass'])) 
    {
        if ($userInfo[0]['u_status']) 
        {
//            $redis = new Redis();
//            $redis->connect('devcache1', 6379); // use your Host from Redis desktop Manager - connection
//
//            $redis->set('u_id', $userInfo[0]['u_id']);
//            $redis->set('u_login', $userInfo[0]['u_login']);
//            ob_start();
            ini_set('session.save_handler', 'redis');
            ini_set('session.save_path', 'tcp://devcache1:6379,tcp://devcache1:6379');


            header('Location: /crud/crud_read.php');
            ob_end_flush();
//            die();
        } 
        else 

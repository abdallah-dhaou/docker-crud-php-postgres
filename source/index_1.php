...
//error_log("\n isLoggedIn ".$isLoggedIn, 3, "/tmp/errors.log");
//die;
if ($isLoggedIn) {
    ...
    if (! empty($_POST["login_signin"])) {
        ...
        if ($isAuthenticated) {
            ...
            if($user[0]["u_status"])
            {
                ...
                error_log("\n status remember".$_POST["remember"], 3, "/tmp/errors.log");
                if (! empty($_POST["remember"])) 
                {
                    ...
                    error_log("\n TEST1", 3, "/tmp/errors.log");
                    // mark existing token as expired
                    $userToken = $auth->getTokenByUsername($username, 0);
                    if (! empty($userToken[0]["id"])) {
                        $auth->markAsExpired($userToken[0]["id"]);
                        error_log("\n TEST2", 3, "/tmp/errors.log");
                    }
                    // Insert new token
                    error_log("\n TEST3", 3, "/tmp/errors.log");
                    $auth->insertToken($username, $random_password_hash, $random_selector_hash, $expiry_date);
                    error_log("\n TEST4", 3, "/tmp/errors.log");
                } 
                else 
                {
                    error_log("\n TEST5", 3, "/tmp/errors.log");
                    $util->clearAuthCookie();
                    error_log("\n TEST6", 3, "/tmp/errors.log");
                }
                $util->redirect("crud/crud_read.php");
            }
            else

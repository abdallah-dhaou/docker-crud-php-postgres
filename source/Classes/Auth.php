...
class Auth {
    function getMemberByUsername($username) {
        ...
        $result = $db_handle->runQuery($query, array($username));
        return $result;
    }
	...
	function getTokenByUsername($username,$expired) {
	    ...
	    $result = $db_handle->runQuery($query, array($username, $expired));
	    return $result;
    }
    ...
    function markAsExpired($tokenId) {
        ...
        $expired = 1;
        $result = $db_handle->update($query, array($expired, $tokenId));
        return $result;
    }
    ...
    function insertToken($username, $random_password_hash, $random_selector_hash, $expiry_date) {
        ...
        $result = $db_handle->insert($query, array($username, $random_password_hash, $random_selector_hash, $expiry_date));
        return $result;
    }
